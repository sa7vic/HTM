...
res = differential_evolution(
    ...
    seed=42, maxiter=600, tol=1e-13,
    mutation=(0.5, 1.5), recombination=0.9,
    popsize=35, workers=-1, polish=True
)
best_params = res.x
best_rmse = res.fun

res2 = minimize(physics_obj, best_params, method='Nelder-Mead',
                options={'maxiter': 12000, 'xatol': 1e-13, 'fatol': 1e-13})
...
spline_stress = UnivariateSpline(
    train["id"].values, stress_train, s=len(train) * 0.45, k=4
)
spline_resid = UnivariateSpline(
    train["id"].values, residual_train, s=len(train) * 1.9, k=3
)
...
print("\n" + "=" * 60)
print("TRAINING 5 DIVERSE LGBMs + XGB × 6 FOLDS = 36 fits")
print("=" * 60)
...
model_configs = [
    {
        ...
        "params": dict(
            ...
            num_leaves=140, subsample=0.86, colsample_bytree=0.81,
            reg_alpha=0.04, reg_lambda=0.09, min_child_samples=2,
    ...
    },
    {
        "name": "LGB-medium",
        "params": dict(
            n_estimators=4000, learning_rate=0.009,
            num_leaves=70, subsample=0.81, colsample_bytree=0.76,
            reg_alpha=0.09, reg_lambda=0.45, min_child_samples=9,
    ...
    },
    {
        ...
        "params": dict(
            n_estimators=5000, learning_rate=0.006,
            num_leaves=210, subsample=0.90, colsample_bytree=0.85,
            reg_alpha=0.018, reg_lambda=0.045, min_child_samples=4,
    ...
    },
    {
        "name": "LGB-wide",
        "params": dict(
            n_estimators=3500, learning_rate=0.011,
            num_leaves=240, subsample=0.88, colsample_bytree=0.83,
            reg_alpha=0.012, reg_lambda=0.025, min_child_samples=5,
            max_bin=511, random_state=456, verbose=-1, n_jobs=-1,
        ),
    },
    {
        "name": "XGB-1",
        "params": dict(
            n_estimators=4000, learning_rate=0.006, max_depth=7,
            subsample=0.85, colsample_bytree=0.80, 
            reg_alpha=0.045, reg_lambda=0.095,
...
for i, cfg in enumerate(model_configs):
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(folds):
        ...
        if "XGB" in name:
            ...
            model.fit(
                ...
                eval_set=[(X_train.iloc[val_idx], y[val_idx])],
                callbacks=[
                    __import__('lightgbm').early_stopping(180, verbose=False),
    ...
    if "XGB" in name:
        model_full = xgb.XGBRegressor(**{k: v for k, v in cfg["params"].items() if k != 'early_stopping_rounds'})
    else:
...
for w0 in np.arange(0.15, 0.65, 0.05):
    for w1 in np.arange(0.05, 0.55, 0.05):
        for w2 in np.arange(0.05, 0.55, 0.05):
            w_rest = 1.0 - w0 - w1 - w2
            if w_rest < 0.05 or w_rest > 0.6:
                continue
            weights = np.array([w0, w1, w2, w_rest * 0.5, w_rest * 0.5])
...
test_blend = 0.42 * (test_preds_cv @ best_weights) + 0.58 * (test_preds_full @ best_weights)
...
test_final = np.clip(test_final, stress_min * 0.89, stress_max * 1.055)
