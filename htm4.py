...
res2 = minimize(physics_obj, best_params, method='Nelder-Mead',
                options={'maxiter': 11000, 'xatol': 1e-12, 'fatol': 1e-12})
...
print("\n" + "=" * 60)
print("TRAINING 5 DIVERSE LGBMs × 5 FOLDS = 25 fits")
print("=" * 60)
...
model_configs = [
    {
        "name": "LGB-deep",
        "params": dict(
            n_estimators=5500, learning_rate=0.0048,
            num_leaves=130, subsample=0.855, colsample_bytree=0.805,
            reg_alpha=0.048, reg_lambda=0.098, min_child_samples=3,
    ...
    },
    {
        "name": "LGB-medium",
        "params": dict(
            n_estimators=3200, learning_rate=0.0098,
            num_leaves=64, subsample=0.802, colsample_bytree=0.752,
            reg_alpha=0.098, reg_lambda=0.49, min_child_samples=10,
    ...
    },
    {
        ...
        "params": dict(
            n_estimators=4200, learning_rate=0.0078,
            num_leaves=198, subsample=0.902, colsample_bytree=0.852,
            reg_alpha=0.022, reg_lambda=0.052, min_child_samples=5,
    ...
    },
    {
        "name": "LGB-light",
        "params": dict(
            n_estimators=2800, learning_rate=0.0122,
            num_leaves=46, subsample=0.775, colsample_bytree=0.715,
            reg_alpha=0.155, reg_lambda=0.68, min_child_samples=12,
            max_bin=255, random_state=999, verbose=-1, n_jobs=-1,
        ),
    },
    {
        "name": "LGB-wide",
        "params": dict(
            n_estimators=3600, learning_rate=0.0088,
            num_leaves=180, subsample=0.88, colsample_bytree=0.83,
            reg_alpha=0.028, reg_lambda=0.075, min_child_samples=6,
            max_bin=383, random_state=456, verbose=-1, n_jobs=-1,
        ),
    },
]
...
for i, cfg in enumerate(model_configs):
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(folds):
        ...
        model.fit(
            ...
            callbacks=[
                __import__('lightgbm').early_stopping(110, verbose=False),
...
def ensemble_objective(weights):
    ...
    return np.sqrt(mean_squared_error(y, oof_blend))
...
test_blend = 0.40 * (test_preds_cv @ best_weights) + 0.60 * (test_preds_full @ best_weights)
